...
/mob/living/simple_animal/hostile/retaliate/gym_rat/UnarmedAttack(var/atom/A)
	if(istype(A, /obj/machinery/disposal) && (last_disposalscavenge + disposalscavenge_cooldown < world.time)) // If we click on a disposal unit, let's look for meat or cheese
		...
		else if(!(last_disposalscavenge + disposalscavenge_cooldown < world.time))
